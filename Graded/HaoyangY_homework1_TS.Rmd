...
Gender male seems to have the highest effect on los.
```
```
...
coef(mod)
```
The estimate for the intercept is 3.5, the estimate for gender male is 4.3.
...
sqrt(diag(b))
```
The standard error for the intercept is 1.099, the standard error for gender male is 1.474.
...
```{r}
sqrt(deviance(mod)/7) # 2.197
summary(mod) # two residual standard errors are the same
...
```{r}
t.test(women$los,men$los) # p-value = 0.02205
t.test(women$los,men$los,var.equal = TRUE) # p-value = 0.02243
```

The t-test with equal variance assumption produce the same p-values as the model summary
